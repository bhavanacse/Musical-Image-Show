...
//delete an image given the image id
	 ...
	 */
	// add music
	public void addMusic(String filePath, String music, String artist,
			long showid) {
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put(KEY_SHOWID, showid);
	...
	public long addMusicGetID(String filePath, String music, String artist,
			long showid) {
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put(KEY_SHOWID, showid);
	...
	public void addMusicList(List<ShowMusic> filePaths, long showid) {
		...
		for (ShowMusic s : filePaths) {
			...
			// insert row
			db.insert(TABLE_IMAGESHOW, null, values);
			System.out.println(showid + " db to add music added "
					+ getMusicCount(showid));
		}
	}

	public int getMusicCount(long slideshow_id) {
		SQLiteDatabase db = this.getWritableDatabase();
		Cursor mCount = db.rawQuery("select count(*) from " + TABLE_SHOWMUSIC
				+ " where showId=" + slideshow_id, null);
		mCount.moveToFirst();
